...
function RegisterForm({ error, onSubmit, onLogin }: RegisterFormProps) {
  ...
  return <form className="auth-form auth-form--register" onSubmit={handleSubmit}>
    ...
    <div className="auth-form__grid"><div><label htmlFor="telefono">Teléfono</label><input id="telefono" type="tel" inputMode="tel" placeholder="70000000" value={data.telefono} onChange={(e) => updateField("telefono", e.target.value)} required /></div><div><label htmlFor="direccion">Dirección / zona</label><input id="direccion" placeholder="Ej. Sopocachi, La Paz" value={data.direccion} onChange={(e) => updateField("direccion", e.target.value)} required /></div></div>
